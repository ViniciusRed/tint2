...
void config_write_gradients(FILE *fp)
{
    ...
    for (GList *gl = gradients ? gradients->next : NULL; gl; gl = gl->next, index++) {
        ...
        fprintf(fp,
                "gradient = %s\n",
                g->type == GRADIENT_CONFIG_HORIZONTAL ? "horizontal"
                : g->type == GRADIENT_CONFIG_VERTICAL ? "vertical"
                                                      : "radial");
...
void config_write_separator(FILE *fp)
{
    for (int i = 0; i < separators->len; i++) {
        ...
        fprintf(fp,
                "separator_style = %s\n",
                gtk_combo_box_get_active(GTK_COMBO_BOX(separator->separator_style)) == 0   ? "empty"
                : gtk_combo_box_get_active(GTK_COMBO_BOX(separator->separator_style)) == 1 ? "line"
                                                                                           : "dots");
        fprintf(fp,
                "separator_size = %d\n",
